...
| `src/lib/radar.server.ts`                               | Server functions + banco. `lerPosicoesBanco()` deve usar o cache em memória (`TTL_BANCO_POSICOES_MS` 10min + dedup de leituras concorrentes); `gravarPosicoesBanco()` atualiza o cache após upsert. Séries do Yahoo passam por `sanitizarPontos` (descarta adjclose quebrado por splits). `contextoMacro()` deduplica requisições em voo. |
| `src/lib/radar.functions.ts`                            | Definições de `createServerFn` do radar (`radarVisao`, `radarPosicoes`, `radarAnaliseIA`, `radarCompletarCache`, ...). Endpoints de custo (`radarAnaliseIA` — LLM, `radarCompletarCache` — até 120 fetches Yahoo/chamada) usam `.middleware([requireSupabaseAuth])` e o limite por usuário `limitePorUsuario()` de `radar.server.ts`.     |
