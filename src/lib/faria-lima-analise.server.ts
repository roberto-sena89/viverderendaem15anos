...
import { montarCandidatos, type CandidatoObservador } from "@/lib/observador-mercado-base";
...
function tesePara(
  c: CandidatoObservador,
  cat: OportunidadeFariaLima["categoriaFariaLima"],
): string {
...
function resumoExecutivoFariaLima(
  ...
  const topTxt = top
    ? `${top.ticker} (${top.score}/100, ${top.percentil?.toFixed(0) ?? "—"}% da história, DY ${top.dy12?.toFixed(1) ?? "—"}%)`
    : "sem líder claro";
  const qtdBaratas = candidatos.filter((c) => (c.percentil ?? 100) <= LIMITE_BAIXA).length;
  const dyMedio = candidatos.length
    ? (candidatos.reduce((s, c) => s + (c.dy12 ?? 0), 0) / candidatos.length).toFixed(1)
    : "—";
  return [
...
export async function analisarFariaLima(): Promise<BriefingFariaLima> {
  ...
  const linhas: LinhaObservador[] = [
    ...(gradeAcoes?.linhas ?? []).map(
      (l: {
        ...
        setor:
          (l as { setor?: string | null }).setor ?? (l as { tipo?: string | null }).tipo ?? null,
        preco: l.preco ?? null,
  ...
  for (const n of feed)
    for (const t of n.tickers)
      noticiasImpacto[t.toUpperCase()] = [...(noticiasImpacto[t.toUpperCase()] ?? []), n.titulo];

  const candidatos = montarCandidatos(linhas, bancoPosicoes.posicoes, noticiasImpacto, 40);
  ...
  if (choque.length)
    alertas.push({
      nivel: "critico",
      titulo: `Choque em ${choque.length} ativos`,
      detalhe:
        choque
          .slice(0, 3)
          .map((c) => `${c.ticker} ${c.variacaoDia?.toFixed(1)}%`)
          .join(", ") + " — valide noticiário antes de comprar.",
    });
  const dyArmadilha = filtrados.filter((c) => (c.dy12 ?? 0) > 12);
  if (dyArmadilha.length)
    alertas.push({
      nivel: "atencao",
      titulo: `DY elevado não recorrente`,
      detalhe: `${dyArmadilha
        .slice(0, 3)
        .map((c) => c.ticker)
        .join(", ")} — cheque provento extraordinário.`,
    });
  if (
    (macroRaw as { selic: number | null }).selic !== null &&
    (macroRaw as { selic: number }).selic > 12
  )
    alertas.push({
      nivel: "atencao",
      titulo: "Selic restritiva",
      detalhe: `Selic ${(macroRaw as { selic: number }).selic}% mantém prêmio de renda fixa alto — exija margem de segurança maior em equity.`,
    });
  ...
  return {
    ...
    macro: {
      selic: (macroRaw as { selic: number | null }).selic,
      ipca: (macroRaw as { ipca: number | null }).ipca,
    },
    universo: {
      acoes: gradeAcoes?.linhas?.length ?? 0,
      fiis: gradeFiis?.linhas?.length ?? 0,
      candidatos: candidatos.length,
    },
    resumoExecutivo: resumoExecutivoFariaLima(macroRaw, candidatos, fiis),
    topValor,
    topDividendos,
    topFiis,
    contrarian,
    alertas,
    metodologia:
      "Score 0-100: preço na história 40% + DY 25% + P/VP 15% + drawdown 10% + ajuste vol/notícia. Filtros: sem penny (<R$1), sem DY>18%, sem choque urgente. Kilo Code (stepfun/step-3.7-flash:free) como motor padrão do Observador.",
  };
}
