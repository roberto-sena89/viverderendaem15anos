...
export default defineTool({
  ...
  description:
    "Registra um novo aporte na carteira do usuário e atualiza (ou cria) o ativo com o novo preço médio.",
  inputSchema: {
    ticker: z.string().describe("Ticker do ativo, ex.: BBAS3."),
    categoria: z
      .string()
      .describe("Categoria do ativo: acoes, fiis, etfs, rendaFixa ou internacional."),
  ...
  handler: async (input, ctx) => {
    ...
    if (quantidade <= 0 || preco <= 0)
      return errorResult("Quantidade e preço devem ser maiores que zero.");
