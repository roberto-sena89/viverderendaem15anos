...
/** Configuraï¿½ï¿½o de um provedor de IA compatï¿½vel com a API OpenAI. */
export interface ConfigProvedorIA {
  ...
  modelo: string;
  /** Chave de API do provedor ï¿½ fica salva apenas neste navegador. */
  chave: string;
...
export interface PresetProvedor {
  ...
  modelos: string[];
  urlChave: string;
  /** Modelos gratuitos verificados para este provedor (o sufixo -free/:free nem sempre ï¿½ confiï¿½vel). */
  modelosGratuitos?: string[];
}

/** Provedores com camada gratuita (free tier) compatï¿½veis com a API OpenAI. */
export const PRESETS_PROVEDOR: PresetProvedor[] = [
  /* 1. OpenRouter */
  {
    id: "openrouter",
    nome: "OpenRouter",
    descricao: "Catï¿½logo com dezenas de modelos gratuitos (sufixo :free). Chave grï¿½tis.",
  ...
  /* 2. OrcaRouter */
  {
    ...
    nome: "OrcaRouter",
    descricao:
      "Roteador aberto que conecta OpenAI, Anthropic, Google e DeepSeek a preï¿½o de custo, com camada gratuita de modelos DeepSeek V4.",
  ...
  /* 4. Nvidia */
  {
    id: "nvidia",
    nome: "Nvidia",
    descricao: "Camada gratuita NVIDIA (build.nvidia.com, com limites de requisiï¿½ï¿½o).",
  ...
  /* 7. Groq */
  {
    id: "groq",
    nome: "Groq Cloud",
    descricao: "Camada gratuita muito rï¿½pida com modelos Compound, GPT-OSS e Qwen.",
  ...
  /* 8. Cline Bot ï¿½ OX Alpha (grï¿½tis) */
  {
    id: "cline",
    nome: "Cline Bot",
    descricao:
      "Gateway Cline Bot (api.cline.bot) ï¿½ OX Alpha (stealth/ox-alpha) com 1M de contexto, gratuito no preview, alï¿½m de 100+ modelos (Claude, GPT, Gemini) via chave ï¿½nica.",
  ...
  /* Hermes Agent */
  {
    id: "nous",
    nome: "Hermes Agent",
    descricao: "B.AI — agregação de modelos (Claude, GPT, Gemini, DeepSeek, Kimi, GLM, MiniMax). Serviço pago por crédito.",
  ...
  /* 9. Kilo Code */
  {
    ...
    nome: "Kilo Code",
    descricao:
      "AI Gateway da Kilo ï¿½ modelos gratuitos (:free) com acesso anï¿½nimo limitado (200 req/hora/IP) ou com chave grï¿½tis.",
  ...
  {
    id: "personalizado",
    nome: "Personalizado (compatï¿½vel OpenAI)",
    descricao: "Qualquer endpoint compatï¿½vel com a API OpenAI (Ollama, LM Studio, etc.).",
    baseUrl: "",
    modelos: [],
    urlChave: "",
  },
...
/** Configuraï¿½ï¿½o padrï¿½o do Gestor IA: Kilo Code (AI Gateway, modelo gratuito). */
...
/**
 * Config vï¿½lida = provedor com URL e modelo preenchidos, e com chave prï¿½pria OU
 * vï¿½nculo com uma variï¿½vel de ambiente do servidor (ex.: Kilo, que aceita
 * acesso anï¿½nimo; a chave entï¿½o fica no servidor e nunca vai para o navegador).
 */
...
/** Cabeï¿½alhos enviados ao /api/chat quando hï¿½ provedor externo configurado. */
...
export async function listarProvedoresBackend(): Promise<ProvedorBackend[]> {
  ...
  if (!session?.access_token) {
    throw new Error("Nï¿½o autenticado");
  }
...
// --------------------------------------------------------------------------
// Histï¿½rico de testes de conexï¿½o (localStorage ï¿½ diagnï¿½stico client-side)
// --------------------------------------------------------------------------

export interface RegistroTesteConexao {
  /** ISO timestamp do teste. */
  timestamp: string;
  /** Nome amigï¿½vel do provedor testado (ex.: "OpenRouter (modelos :free)"). */
  provedor: string;
  /** true = sucesso, false = falha. */
  ok: boolean;
  /** Cï¿½digo HTTP retornado (0 para erros de rede). */
  status: number;
  /** Resumo do resultado (ex.: "Conexï¿½o validada ï¿½ 42 modelos"). */
  resumo: string;
}
...
/** Adiciona um registro ao histï¿½rico (mantï¿½m os ï¿½ltimos 12, mais novos primeiro). */
export function registrarTeste(registro: Omit<RegistroTesteConexao, "timestamp">) {
  ...
  try {
    ...
    /* ignore ï¿½ sï¿½ diagnï¿½stico client-side */
  }
}
