...
export function DashboardPreview({ className }: DashboardPreviewProps) {
  ...
  return (
    <div
      className={cn(
        "bg-background flex h-full flex-col overflow-hidden text-foreground",
        className,
      )}
    >
      {/* Header simulado */}
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-card/80 border-border/40 rounded-xl border p-3">
            ...
            <div className="mt-1 flex items-center justify-between">
              ...
              <span className="text-[0.45rem] font-medium text-primary/90 dark:text-primary/90">
                +R$ 93.420
              </span>
            </div>
          </div>
          ...
          <div className="bg-card/80 border-border/40 rounded-xl border p-3">
            <div className="flex items-center gap-1.5 text-muted-foreground">
              ...
              <span className="text-[0.5rem] font-medium uppercase tracking-wider">
                Lucro Total
              </span>
            </div>
          ...
          <div className="bg-card/80 border-border/40 rounded-xl border p-3">
            ...
            <span className="mt-1 block text-[0.45rem] font-medium text-primary/90 dark:text-primary/90">
              YoC: 8,4%
            </span>
          </div>
        </div>
            ...
            {[
              ...
              <div key={ativo.ticker} className="flex items-center justify-between text-[0.55rem]">
                ...
                <div className="text-right flex flex-col gap-0.5">
                  ...
                  >
                    {ativo.lucro >= 0 ? "+" : "-"} R${" "}
                    {Math.abs(ativo.lucro).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                  </p>
                  <p className="text-[0.45rem] font-bold uppercase tracking-wider text-muted-foreground/90">
                    <span
                      className={cn(ativo.lucroPct >= 0 ? "text-positive/80" : "text-negative/80")}
                    >
                      {ativo.lucroPct >= 0 ? "+" : ""}
                      {ativo.lucroPct}%
                    </span>
